...
for i in range(0, 16):
    for j in range(0, 16):
        # find horizontal products
        temp = grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3]
        if temp >= largest_product:
            largest_product = temp

        # find vertical products
        temp = grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j]
        if temp >= largest_product:
            largest_product = temp

        # find upper-left to lower-right products
        temp = grid[i][j] * grid[i + 1][j + 1] * \
            grid[i + 2][j + 2] * grid[i + 3][j + 3]
        if temp >= largest_product:
            largest_product = temp

        # find lower-left to upper-right products
        temp = grid[i][j + 3] * grid[i + 1][j + 2] * \
